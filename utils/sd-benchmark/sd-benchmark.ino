...
void createFiles() {
  ...
  for (uint8_t i = 0; i < MAX_FILES; i++) {
    ...
    if (!file[i].open(name, O_RDWR | O_CREAT | O_TRUNC))
      error("open create");
    if (!file[i].preAllocate(FILE_ALLOC))
      error("preAllocate");
  }
  m = micros() - m;
  ...
  for (uint8_t i = 0; i < MAX_FILES; i++) {
    for (uint32_t iw = 0; iw < nw; iw++) {
      if (sizeof(buf) != file[i].write(buf, sizeof(buf)))
        error("create write");
    }
  }
  m = micros() - m;
    ...
    for (uint8_t nf = 1; nf <= MAX_FILES; nf *= 2) {
      ...
      for (uint32_t ir = 0; ir < nr; ir++) {
        for (uint32_t i = 0; i < nf; i++) {
          lat = micros();
          if (ns * 512 != file[i].read(buf, ns * 512))
            error("randomRead");
          lat = micros() - lat;
          if (lat > maxLat) {
            maxLat = lat;
  ...
  for (uint8_t ns = MAX_SECTORS; ns > 0; ns /= 2) {
    for (uint8_t nf = 1; nf <= MAX_FILES; nf *= 2) {
      ...
      for (uint32_t ir = 0; ir < nr; ir++) {
        for (uint32_t i = 0; i < nf; i++) {
          lat = micros();
          if (ns * 512 != file[i].write(buf, ns * 512))
            error("randomWrite");
          lat = micros() - lat;
          if (lat > maxLat) {
            maxLat = lat;
